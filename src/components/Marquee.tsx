...
export function MarqueeFade({ side = 'left', className }: MarqueeFadeProps) {
  return (
    <div
      className={cn(
        'pointer-events-none absolute top-0 z-10 h-full w-32',
        side === 'left' 
          ? 'left-0 bg-gradient-to-r from-[var(--bg-color)] via-[var(--bg-color)]/80 to-transparent' 
          : 'right-0 bg-gradient-to-l from-[var(--bg-color)] via-[var(--bg-color)]/80 to-transparent',
        className
      )}
    />
  );
